...
export default function AdminAnalyticsPage() {
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-16 print:p-0 print:space-y-4">
      ...
      {/* Control Panel: Period Mode Selector & Date Customizers */}
      <div className="glass-card rounded-3xl p-5 border border-slate-200/70 bg-white shadow-xs space-y-4 print:hidden">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-1.5 p-1.5 bg-slate-100/80 rounded-2xl border border-slate-200/60 self-start sm:self-auto">
            <button
              type="button"
          ...
          <div className="flex flex-wrap items-center gap-1.5 text-xs">
            <span className="text-slate-400 font-bold text-[10px] uppercase tracking-wider mr-1">
              {t('Quick Jump:', 'اختصارات:')}
            </span>
            <button
              type="button"
              onClick={handleSetThisMonth}
              className="px-2.5 py-1 rounded-lg text-slate-600 bg-slate-50 hover:bg-slate-100 border border-slate-200/70 font-semibold cursor-pointer transition"
            >
          ...
          {periodMode === 'MONTH' && (
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-slate-400 font-medium">
                {t(
                  'Shows daily trend breakdown for the chosen month',
                  'عرض إحصائيات المبيعات اليومية للشهر المحدد'
                )}
              </span>
            </div>
          )}
