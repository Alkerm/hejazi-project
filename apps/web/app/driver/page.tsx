...
import { Truck, Phone, MessageSquare, MapPin, CheckCircle2, RefreshCw, UserCheck } from 'lucide-react';
...
export default function DriverDashboardPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const [avail, mine] = await Promise.all([
        ...
        api.getMyAssignedDeliveries(driverName),
      ]);
      setAvailableOrders(avail);
      setMyOrders(mine);
    } catch (err: any) {
      toast.error(err.message || 'Failed to load driver manifest');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleClaimOrder = async (orderId: string) => {
    try {
      ...
      toast.success('Order claimed! Moved to your active delivery list.');
      await loadData();
  ...
  const handleCompleteDelivery = async (orderId: string) => {
    try {
      ...
      toast.success('Order marked DELIVERED and payment updated!');
      await loadData();
    } catch (err: any) {
      toast.error(err.message || 'Failed to mark delivery completed');
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-lg mx-auto pb-20 px-4">
      ...
      <div className="glass-card rounded-2xl p-5 border border-slate-200/50 space-y-4 shadow-sm bg-slate-900 text-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="font-bold text-lg leading-tight">Hejazi Express Driver</h1>
              <p className="text-[10px] text-slate-400 uppercase tracking-widest">Mobile Dispatch Portal</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 gap-2 p-1 bg-slate-200/60 rounded-xl text-xs font-bold">
        ...
        >
          Ready to Pickup ({availableOrders.length})
        </button>
      </div>
        ...
        <div className="space-y-4">
          {myOrders.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-8 text-center text-slate-500 text-xs space-y-2">
              ...
              <p className="font-bold text-slate-700">No active deliveries claimed yet</p>
              <p>Switch to "Ready to Pickup" tab to claim orders ready for delivery.</p>
            </div>
          ) : (
            myOrders.map((order) => {
              ...
              return (
                <div key={order.id} className="glass-card rounded-2xl p-5 border border-slate-200/50 space-y-4 bg-white shadow-sm">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                    <div>
                      <span className="font-bold text-sm text-slate-800">Order #{order.id.slice(-8)}</span>
                    ...
                    <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${
                      ...
                      {order.paymentStatus === 'PAID' ? 'PAID ONLINE' : `COLLECT ${formatMoney(order.total, order.currency)} COD`}
                    </span>
                  </div>
        ...
        <div className="space-y-4">
          {availableOrders.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-8 text-center text-slate-500 text-xs">
              No orders ready for delivery pickup right now.
            </div>
          ) : (
            availableOrders.map((order) => (
              <div key={order.id} className="glass-card rounded-2xl p-5 border border-slate-200/50 space-y-3 bg-white">
                ...
                <div className="text-xs text-slate-600 space-y-1">
                  <p><strong className="text-slate-700">Destination:</strong> {order.shippingAddressSnapshot.city}</p>
                  <p><strong className="text-slate-700">Items:</strong> {order.items.length} cosmetic products</p>
                </div>
